(() => {
  ...
  function clearError(rule) {
    ...
    container.querySelector(`#${errorId(rule)}`)?.remove();
  ...
  function showError(rule) {
    ...
    if (rule.type === 'choice') {
      ...
      if (help?.nextSibling) container.insertBefore(error, help.nextSibling);
  ...
  function firstInvalid(stepNumber) {
    ...
    stepRules.forEach(rule => {
      if (isValid(rule)) clearError(rule);
      else if (!invalid) invalid = rule;
    });
  ...
  function handleFieldUpdate(event) {
    ...
    const rule = allRules.find(item =>
      item.type === 'text' ? item.id === target.id : item.name === target.name
    );
    if (rule) clearIfValid(rule);
  }
