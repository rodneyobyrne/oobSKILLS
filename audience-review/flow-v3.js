(() => {
  ...
  if (!form || !page || !stage || !progress || !submitButton) return;

  const steps = [1, 2, 3].map(number => stage.querySelector(`.step[data-step="${number}"]`));
  ...
  function validateStep(stepNumber) {
    ...
    if (stepNumber === 1) {
      ...
      if (!offer?.value.trim()) return showMissingField(offer?.closest('.field'), 'Tell us what product, service, or professional role you want us to review.');
      if (!questionSelected('offer_type')) return showMissingField(fieldContainerFor('offer_type'), 'Choose the option that best describes your work.');
      return true;
    }
    ...
    if (stepNumber === 2) {
      ...
      if (!questionSelected('audience_emotions')) return showMissingField(fieldContainerFor('audience_emotions'), 'Select what your customers are likely feeling at this point.');
  ...
  function buildPayload() {
    return {
      ...
      audienceEvidence: {
        ...
        emotionalState: selectedValuesFor('audience_emotions'),
        decisionNeeds: selectedValuesFor('audience_needs'),
        resistanceSignals: selectedValuesFor('audience_hesitation'),
        desiredMovement: selectedValuesFor('audience_outcome')
      },
      businessEvidence: {
        providerValues: selectedValuesFor('business_values'),
        intendedMessage: selectedValuesFor('business_message')
      },
